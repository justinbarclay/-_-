...
I realize that I am in a unique spot though. Most of Emacs' text manipulation is meant to be in place. But, I want to: copy a region/buffer, mutate some text, provide this text as a return value from a function, and not mutate or change the current buffer. Emacs has all of the tools to do this and some of these tools are just easier to find than others.
