...
Using [[https://www.youtube.com/watch?v=fTvQTMOGJaw][resclient]] as my primary means to play with APIs worker for me for quite awhile. It was a simple to read document, that housed all the requests in a single file, and lived alongside the rest of my code in `git`.
...
There was a simple solution for my first and last points, which was to use [[https://www.youtube.com/watch?v=GK3fij-D1G8][org-mode]]. Org-mode has a [[https://orgmode.org/worg/org-contrib/babel/][babel]] extension for [[https://github.com/alf/ob-restclient.el][restclient]], making it easy for me to write in a more literate style.
  ...
  4. However when I'm exploring APIs, first or third parties, I do it in a much more interactive manor: I'll take try stuff, I'll take notes, I'll cluster my API calls, I'll pre or post process the data. For me, this is a practice that lends it self a [[https://hub.mybinder.org/user/binder-examples-julia-python-h6xqlrbg/notebooks/julia.ipynb][notebook]] style of development.

  5. However, as an Emascist `org-mode`, and [[https://github.com/alf/ob-restclient.el][ob-restclient]], we can automate process further and share are work with out coworkers. I mean, that is if our coworkers had good taste and used Emacs over Vim, or Atom, or VSCode.
