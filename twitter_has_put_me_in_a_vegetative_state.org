...
Before we can do all of the fun authentication bits that is ~OAuth2~, we need to have some content. So, I feel like I need to be on brand for an Emacs user and let everyone know I am using Emacs for something that isn't editing text.
#+NAME: hello-world
#+BEGIN_SRC emacs-lisp
...
Finally, now that we've done all that work to formattin and signing things, we can finish it off by tweeting to the world how much we love Emacs.
...
We can use all of the source blocks we created back when we were professing our love for Emacs. However, we need to change a few reference. In the source block below we need to change the reference from ~body=hello-world~ to ~body=twitter-plant-body~.
