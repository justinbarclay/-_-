* Literate Programming with Orgmode and restclient                    :TOC_4:
...
I don't need to store the authentication information in files, and I am to lazy to remember or copy and paste then! I can just use the information that is stored in my environment.
...
Finally, now that we've done all that work to sign things, we can finish it off by tweeting to the world how much we love Emacs.
...
Why, of course we can! This is Emacs, we pretty much have to do something overly complicated and elaborate.
...
1. To grab a plant's name from trefle.io
...
Similiarly, we need to reassign a different variable to twitter-oauth-headers, ~twitter-oauth-headers-1~
